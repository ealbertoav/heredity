...
def powerset(s):
    """
    Return a list of all possible subsets of a set s.
    """
    s = list(s)
    return [
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    def passing_probability(parent):
        """Calculate the probability that a parent passes the gene to a child."""
        if parent in two_genes:
            # Parent has 2 genes, passes gene unless mutation
            return 1 - PROBS["mutation"]
        elif parent in one_gene:
            # Parent has 1 gene, 50% chance of passing either copy
            return 0.5
        else:
            # Parent has 0 genes, passes gene only if a mutation occurs
    ...
    for person in people:
        ...
        if mother is None and father is None:
            ...
            # Has parents - calculated inheritance probability
            prob_from_mother = passing_probability(mother)
            prob_from_father = passing_probability(father)

            if num_genes == 0:
                # Neither parent passes gene
                gene_prob = (1 - prob_from_mother) * (1 - prob_from_father)
            elif num_genes == 1:
                # Exactly one parent passes gene
